...
Korean:
- AES SubBytes 단계에서 사용하는 8-bit S-box lookup module입니다.
- 8-bit 입력 byte 하나를 받아서 AES 표준 S-box 값으로 치환합니다.
- 이 모듈은 조합논리(combinational logic)이므로 clk/reset이 필요 없습니다.
- 나중에 subbytes.v에서 이 sbox module을 16개 사용하여 128-bit state의 모든 byte를 변환합니다.

English:
- This is an 8-bit AES S-box lookup module used in the SubBytes step.
- It takes one 8-bit input byte and substitutes it with the standard AES S-box value.
- This is combinational logic, so no clock or reset is required.
- Later, subbytes.v will instantiate 16 copies of this module to transform all bytes in a 128-bit state.
*******************************************************************************************************
*/

module sbox (
    input  wire [7:0] in_byte,   // Korean: S-box 입력 byte / English: input byte to the S-box
    output reg  [7:0] out_byte   // Korean: S-box 출력 byte / English: substituted output byte
);

    /*
    Korean:
    - AES S-box는 고정된 lookup table입니다.
    - 입력값 in_byte에 따라 정해진 out_byte를 출력합니다.
    - always @(*)는 입력이 바뀔 때마다 output이 즉시 다시 계산된다는 뜻입니다.

    English:
    - The AES S-box is a fixed lookup table.
    - The output byte depends on the input byte.
    - always @(*) means the output is recalculated whenever the input changes.
    */
    always @(*) begin
        case (in_byte)
            ...
            // Korean:
            // 모든 8-bit 값은 위 case에 포함되어 있지만,
            // latch 생성을 방지하기 위해 default를 넣습니다.
            //
            // English:
            // All 8-bit values are covered above,
            // but default is included to avoid unintended latch inference.
